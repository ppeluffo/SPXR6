...
{
	/*
	 * Determina en los canales configurados si hay alguno ( analogico, contador o modbus )
	 * que mida el caudal.
	 * Para esto lee los nombres y busca q{x}, Q{X}, CAU{}
	 * Una vez que encuentra uno, sale: no busca mas
	 */
...
quit:
	...
	if ( qChannel.tipo == MODBUS ) {
		xprintf_P(PSTR("CAUDAL: Canal %d MODBUS\r\n"), qChannel.pos );
	} else if ( qChannel.tipo == ANALOG ) {
		xprintf_P(PSTR("CAUDAL: Canal %d ANALOG\r\n"), qChannel.pos );
	} else 	if ( qChannel.tipo == COUNTER ) {
