...
//	xprintf_P(PSTR("DEBUG:RTC_read: rdAddress=0x%02x, length=0x%02x\r\n"), rdAddress, length);

	I2C_get_semaphore();
	rcode =  I2C_read( BUSADDR_RTC_M79410, rdAddress, 1, data, length );
	if ( rcode == -1 ) {
...
//	xprintf_P(PSTR("DEBUG:RTC_write: wrAddress=0x%02x, length=0x%02x\r\n"), wrAddress, length);

	I2C_get_semaphore();
	rcode =  I2C_write ( BUSADDR_RTC_M79410, wrAddress, 0x01, data, length );
	I2C_release_semaphore();
